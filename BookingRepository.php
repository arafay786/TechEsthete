...
class BookingRepository extends BaseRepository
{
    ...
    
    {
        ...
        {
            $jobs = $cuser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'user.language', 'user.feedback')->whereIn('status', ['pending', 'assigned', 'started'])->orderBy('due', 'asc')->get();
            $usertype = 'customer';
        } 
    ...
    
    {
        ...
        {
            $jobs = $cuser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'user.language', 'user.feedback', 'user.distance')->whereIn('status', ['completed', 'withdrawbefore24', 'withdrawafter24', 'timedout'])->orderBy('due', 'desc')->paginate(15);
            $usertype = 'customer';
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    
    {
        ...
        Event::fire(new SessionEnded($job, ($post_data['userid'] == $job->userId) ? $tr->userId : $job->userId));
    ...
    
    {
        ...
        foreach ($users as $oneUser)
         ...
         {
            if ($oneUser->userType == '2' && $oneUser->status == '1' && $oneUser->id != $exclude_userId)
        ...
        $this->sendPushNotificationToSpecificUsers($delpayTranslatorArray, $job->id, $data, $msg_text, true); // send new booking push to suitable translators(need to delay)
    }
     ...
     * @param $isNeedDelay
     */
    public function sendPushNotificationToSpecificUsers($users, $jobId, $data, $msg_text, $isNeedDelay)
    
    {
        ...
        $logger->pushHandler(new StreamHandler(storage_path('logs/push/laravel-' . date('Y-m-d') . '.log'), Logger::DEBUG));
        $logger->pushHandler(new FirePHPHandler());
        $logger->addInfo('Push send for job ' . $jobId, [$users, $data, $msg_text, $isNeedDelay]);
        if (env('APP_ENV') == 'prod') 
        {
...
//        }
    ...
    
    {
        ...
        if (!is_null($currentTranslator) || (isset($data['translator']) && $data['translator'] != 0) || $data['translator_email'] != '')
         ...
         {
            $log_data = [];
            if (!is_null($currentTranslator) && ((isset($data['translator']) && $currentTranslator->userId != $data['translator']) || $data['translator_email'] != '') && (isset($data['translator']) && $data['translator'] != 0))
    ...
    
    {
        ...
        $data['customer_phone_type'] = $job->customer_phone_type;
        $data['customer_physical_type'] = $job->customer_physical_type;
        $data['customer_town'] = $userMeta->cMy;
...
//                Event::fire(new JobWasCanceled($job));
    ...
    
    {
        ...
        $jobType = 'unpaid';
        $translatorType = $cuserMeta->translatorType;
        if ($translatorType == 'professional')
            $jobType = 'paid';   /*show all jobs for professionals.*/
        else if ($translatorType == 'rwstranslator')
            $jobType = 'rws';  /* for rwstranslator only show rws jobs. */
        ...
        $languages = UserLanguages::where('userId', '=', $cuser->id)->get();
        $userlanguage = collect($languages)->pluck('lang_id')->all();
...
//        $jobs = TeHelper::convertJobIdsInObjs($jobIds);
    ...
    
    {
        ...
        $consumerType = $cuser->consumerType;

        if ($cuser && $cuser->userType == env('SUPERADMIN_ROLE_ID')) 
        {
            $allJobs = Job::query();
            ...
            if (isset($requestdata['consumerType']) && $requestdata['consumerType'] != '') 
    ...
    
    {
        ...
        $requestdata = Request::all();
        $all_customers = DB::table('users')->where('userType', '1')->lists('email');
        $all_translators = DB::table('users')->where('userType', '2')->lists('email');

        $cuser = Auth::user();
        $consumerType = TeHelper::getUsermeta($cuser->id, 'consumerType');
